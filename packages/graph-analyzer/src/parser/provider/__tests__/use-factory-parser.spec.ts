...
describe("UseFactoryParser", () => {
	describe("Basic Functionality", () => {
		...
		it("should parse factory with inject array", () => {
			const provider =
				'{ provide: "SERVICE", useFactory: (config) => { return new Service(config); }, inject: [ConfigService, DatabaseService] }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle whitespace in provider string", () => {
			const provider =
				'{  provide:  "FACTORY"  ,  useFactory:  () => { return {}; }  }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
	...
	describe("Token Extraction", () => {
		it("should extract token from provide field", () => {
			const provider =
				'{ provide: "DATABASE_CONNECTION", useFactory: () => { return db; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle token with underscores", () => {
			const provider =
				'{ provide: "API_CLIENT", useFactory: () => { return client; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle token with numbers", () => {
			const provider =
				'{ provide: "SERVICE_V2", useFactory: () => { return service; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
	...
	describe("Factory Function Extraction", () => {
		it("should extract simple arrow function", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: () => { return value; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should extract factory with parameters", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (dep1, dep2) => { return new Service(dep1, dep2); } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should extract factory with complex body", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (config) => { const service = new Service(); service.init(config); return service; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
	...
	describe("Inject Array Extraction", () => {
		it("should extract single inject dependency", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (dep) => { return dep; }, inject: [ConfigService] }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should extract multiple inject dependencies", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (a, b, c) => { return {}; }, inject: [ServiceA, ServiceB, ServiceC] }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle inject with whitespace", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: () => {}, inject: [ ServiceA , ServiceB , ServiceC ] }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
	...
	describe("Edge Cases", () => {
		...
		it("should handle provider with extra fields", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: () => {}, scope: "singleton", inject: [Service] }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
	...
	describe("Chaining", () => {
		...
		it("should allow method chaining", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: () => {}, inject: [Service] }';
			const parser = new UseFactoryParser(provider);
	...
	describe("Complex Scenarios", () => {
		it("should handle factory returning object literal", () => {
			const provider =
				'{ provide: "CONFIG", useFactory: () => { return { key: "value" }; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle factory with nested functions", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (service) => { return service.getData(); } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
		...
		it("should handle factory with conditional logic", () => {
			const provider =
				'{ provide: "TOKEN", useFactory: (env) => { return env === "prod" ? prodService : devService; } }';
			const parser = new UseFactoryParser(provider);
			parser.parse();
