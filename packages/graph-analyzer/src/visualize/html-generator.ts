...
export class HtmlGenerator {
	...
	private convertToCytoscapeFormat(graphData: GraphOutput): {
		...
		for (const provider of graphData.providers) {
			...
			const hasMissingDecorators = provider.dependencies?.some(
		...
		return { nodes, edges, providerMap: providerMapObject, moduleMap: moduleMapObject };
	}
	...
	private generateMissingDecoratorsWarning(graphData: GraphOutput): string {
		const providersWithMissingDecorators = graphData.providers.filter(provider =>
			provider.dependencies?.some(dep => dep.hasExplicitDecorator === false)
		);
		...
		const providersList = providersWithMissingDecorators.map(provider => {
			const missingDeps = provider.dependencies?.filter(dep => dep.hasExplicitDecorator === false) || [];
			return `<li><strong>${provider.token}</strong> (${provider.module}): ${missingDeps.map(d => d.token).join(', ')}</li>`;
		}).join('');

		return `
            <div style="background: #ffebee; border: 2px solid #f44336; border-radius: 4px; padding: 1rem; margin-top: 1rem;">
                <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
                    <span style="font-size: 1.5rem;">⚠️</span>
                    <strong style="color: #c62828; font-size: 1.1rem;">Missing @Inject Decorators Detected</strong>
                </div>
                <p style="color: #c62828; margin-bottom: 0.75rem; font-size: 0.9rem;">
                    ${providersWithMissingDecorators.length} provider(s) have dependencies without explicit @Inject decorators.
                    This may cause runtime errors in production. Affected providers:
                </p>
                <ul style="color: #c62828; font-size: 0.875rem; margin-left: 1.5rem;">
                    ${providersList}
                </ul>
            </div>
        `;
	...
	private generateHtmlTemplate(
		...
		cytoscapeData: { nodes: unknown[]; edges: unknown[]; providerMap: Record<string, string>; moduleMap: Record<string, string> },
	): string {
