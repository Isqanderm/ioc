...
describe("HtmlGenerator", () => {
	...
	describe("generate method", () => {
		...
		it("should apply light theme by default", () => {
			...
			// Light theme colors
			expect(html).toContain("#f8f9fa"); // Light background
			expect(html).toContain("#2c3e50"); // Dark text
		});
	});
	...
	describe("Interactive Navigation", () => {
		...
		it("should highlight target node on navigation", () => {
			...
			// The new implementation uses selective edge filtering with show()
			expect(html).toContain(".show().addClass('highlighted')");
		});
