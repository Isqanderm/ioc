...
export class CommentRepository {
	...
	async create(data: CreateCommentDto): Promise<Comment> {
		this.logger.info(
			`Creating comment on post: ${data.postId}`,
			"CommentRepository",
		);
	...
	async findByPost(postId: string): Promise<Comment[]> {
		this.logger.debug(
			`Finding comments for post: ${postId}`,
			"CommentRepository",
		);
	...
	async findByAuthor(authorId: string, limit = 10): Promise<Comment[]> {
		this.logger.debug(
			`Finding comments by author: ${authorId}`,
			"CommentRepository",
		);
	...
	async findReplies(parentId: string): Promise<Comment[]> {
		this.logger.debug(
			`Finding replies to comment: ${parentId}`,
			"CommentRepository",
		);
